...
const Exchanges = forwardRef<HTMLDivElement>((props, ref) => (
  <section 
    id="exchanges" 
    ref={ref} 
    className="relative flex flex-col items-center bg-black text-white min-h-screen py-32 px-6"
  >
    ...
    {/* Exchange Grid */}
    <motion.div 
      className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-4 mt-24 w-full max-w-6xl"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      {exchanges.map((exchange, index) => (
        <motion.a 
          ...
          className="relative bg-black bg-opacity-60 rounded-xl p-6 flex flex-col md:flex-col lg:flex-col items-center text-center shadow-lg shadow-pink-500/30 border border-pink-500 hover:border-pink-400 transition duration-300 transform hover:-translate-y-2 w-full max-w-md"
        ...
        >
          ...
          <div className="mt-2">
            <p className="text-lg font-semibold">{exchange.name}</p>
            <p className="text-sm text-gray-300">{exchange.network}</p>
          </div>
        </motion.a>
      ))}
    </motion.div>
